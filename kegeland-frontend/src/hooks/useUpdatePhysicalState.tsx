...
const useUpdatePhysicalState = () => {
  ...
  const updatePhysicalState = useCallback(async (patientId: string, physicalStateData: CurrentPhysicalState) => {
    setLoading(true);
    setError(null);
    try {
      const resultAction = await dispatch(updatePatientData({
        pid: patientId,
        currentPhysicalState: physicalStateData,
      }));
      unwrapResult(resultAction); // If the promise rejects, this will throw
    } catch (error: any) { // if using TypeScript, you can specify `unknown` instead of `any`
